...
def add_product():
    ...
    if 'name' in data and 'price' in data:
        products = Product(name=data["name"], price=data["price"], description=data.get("description", ""))
...
def delete_product(product_id):
    ...
    if product:
        ...
        db.session.commit()
        return jsonify({"message": "Product deleted successsfully"})
    return jsonify({"message": "Product not found"}), 404
...
def logout():
    logout_user()
    return jsonify({"message": "Logout in successsfully"})
